...
mod tests {
    ...
    use unit_compose_core::{
        ...
        DiagnosticSink, ExecutableDefinition, HostOutput, Module, ModuleInputs, ParsedModule,
        ParsedUnit, PortDescriptor, ResourceDescriptor, ResourceId, ResourceRegistry,
        ResourceRequirement, RunError, RunEvent, RunEventKind, SemanticType, UnitDescriptor,
        UnitFailure, UnitId, UnitRegistry, UnitRequirements, UnitTypeName,
    };
    ...
    impl DiagnosticSink for AllocatingSink {
    ...
    fn isolated_dynamic_strict_allocation_conformance() {
        ...
        assert_eq!(
            allocating.report().events().next().unwrap().kind,
            RunEventKind::AllocationProfileViolation
        );
        assert!(allocating.output(&output).is_err());
    }

    #[test]
    fn profiled_run_measures_diagnostic_sink_allocations() {
        let inputs = ModuleInputs::default();
        let mut module = prepared(false, None, false);
        let mut probe = GlobalProbe;
        let mut sink = AllocatingSink::default();

        assert!(matches!(
            module.run_profiled(&inputs, &mut [&mut probe], Some(&mut sink)),
            Err(RunError::AllocationProfileViolation { .. })
        ));
    ...
    #[test]
    fn profiled_run_rejects_duplicate_allocation_domains_before_measurement() {
        let inputs = ModuleInputs::default();
        let mut module = prepared(false, None, false);
        let mut first = GlobalProbe;
        let mut second = GlobalProbe;

        let error = module
            .run_profiled(&inputs, &mut [&mut first, &mut second], None)
            .unwrap_err();

        assert!(matches!(
            error,
            RunError::AllocationProfileViolation { domain, operations }
                if domain == "rust-global" && operations.is_zero()
        ));
        assert_eq!(module.report().events().count(), 0);
    }

    #[test]
    fn profiled_recoverable_failure_keeps_context_without_measured_allocations() {
        let inputs = ModuleInputs::default();
        let mut module = prepared(
            false,
            Some(unit_compose_core::FailureDisposition::Recoverable),
            false,
        );
        ...
        assert!(matches!(error.root_cause(), RunError::Unit(_)));
        assert_eq!(error.context().unwrap().module, "allocation-fixture");
        assert_eq!(
            error.context().unwrap().unit.as_ref().unwrap().as_str(),
            "source"
        );
        assert_eq!(
            module.report().allocation_operations(),
    ...
    #[test]
    fn profiled_fatal_failure_remains_the_primary_error_when_it_also_allocates() {
        let inputs = ModuleInputs::default();
        let mut module = prepared(
            true,
            Some(unit_compose_core::FailureDisposition::Fatal),
        ...
        assert!(matches!(error.root_cause(), RunError::Unit(_)));
        assert_eq!(
            error.context().unwrap().disposition,
            Some(unit_compose_core::FailureDisposition::Fatal)
        );
        assert_ne!(
            module.report().events().next().unwrap().kind,
            RunEventKind::AllocationProfileViolation
        );
        assert!(!module.report().allocation_operations().is_zero());
        assert!(matches!(
            module.run(&inputs).unwrap_err().root_cause(),
            RunError::Poisoned
        ));
    }
