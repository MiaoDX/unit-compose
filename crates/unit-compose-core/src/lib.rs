...
impl ResourceDescriptor {
    ...
    /// Describes a fixed-length typed buffer backed by `Vec<E>`.
    pub fn fixed_buffer<E: 'static>(
        semantic_type: SemanticType,
        adapter: &'static str,
        validation: &'static str,
    ) -> Self {
    ...
    /// Describes a bounded variable-length typed buffer backed by `Vec<E>`.
    pub fn bounded_buffer<E: 'static>(
        semantic_type: SemanticType,
        adapter: &'static str,
        validation: &'static str,
    ) -> Self {
    ...
    fn buffer<E: 'static>(
        ...
        Self {
            semantic_type,
            concrete_type: TypeId::of::<Vec<E>>(),
            concrete_name: type_name::<Vec<E>>(),
...
impl UnitExecutionRecorder {
    ...
    fn module_clock_reads(&self) -> u8 {
        let measured_units = self.len.saturating_add(self.dropped_events);
        2_u8.saturating_add(
            u8::try_from(measured_units)
                .unwrap_or(u8::MAX)
                .saturating_mul(2),
        )
    }
...
impl RunReport {
    ...
    fn replace_last(&mut self, event: RunEvent) {
        if let Some(last) = self.events[..self.len].last_mut() {
            *last = Some(event);
        } else {
            self.push(event);
        }
...
pub trait DiagnosticSink {
    ...
    /// Corrects the event most recently recorded for the current run.
    /// Sinks that retain events should replace in place; the default preserves
    /// the final outcome by recording a second event.
